...
AI 对话只会把已选表的 Schema 与文件元数据、当前会话消息，以及满足绑定一致性条件后显式启用的 SQL/结果样本发送给配置的 AI 服务。包含敏感列名、字段值或业务信息的工作区应使用获准的服务地址；不选择表格时仅发送纯对话内容。公网服务默认必须使用 HTTPS，并拒绝回环、私网、链路本地和保留地址；仅在单机受信任环境显式设置 `ANYDATAS_AI_ALLOW_PRIVATE_NETWORK=1` 后才允许局域网模型。
